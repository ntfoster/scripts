...
def process_pdf(input): # parse TOC and create CSV
    ...
    if "high" or "low" in args.quality:
        ...
        for f in args.formats:
            # add bookmarks
            original_pdf = os.path.splitext(input)[0]+'_'+f+'_screen.pdf'
            if f == "norules":
